...
struct BorderBeamEffect: ViewModifier {
   ...
   private func BorderBeamView() -> some View {
        ZStack {
            ...
            // use keyframe animator to animate the border beam
            KeyframeAnimator(initialValue: 0.0, repeating: true) {
                ...
                RoundedRectangle(cornerRadius: cornerRadius)
                    .fill(beamGradient)
                //inverse masking to show only some limited amount beam gradient
                    .mask {
                        Rectangle()
                            .overlay {
                                Rectangle()
                    ...
                    .mask {
                        // masking it with the already having border gradient to async with the border effect
                        RoundedRectangle(cornerRadius: cornerRadius)
                            .fill(borderGradient)
